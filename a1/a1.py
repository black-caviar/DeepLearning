...
#helper function for plotting basis functions 
...
for i in range(num_iter):
    ...
    #update parameters with gradient scaled by rate
    params['w'].assign(params['w'] - rate*grad['w'])
    params['b'].assign(params['b'] - rate*np.divide(params['b'].numpy(), grad['b'].numpy()))
    #scale b by itself because otherwise result is unreasonable
...
#plot basis functions after optimization
plot_basis(M, params, clean_x)
